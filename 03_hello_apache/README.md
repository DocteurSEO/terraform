...
4. **Réseau** : le port **80** doit être joignable. **Question de sécurité** (à traiter, pas juste à subir) : à quelle **plage source** ouvres-tu le port 80, et pourquoi `0.0.0.0/0` est-il risqué ? Le port **22 (SSH)** est-il *vraiment* nécessaire pour réussir ce test ?
