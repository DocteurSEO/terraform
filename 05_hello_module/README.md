...
- `main.tf` (racine) : le fichier d'**assemblage**. Il contient une **erreur de syntaxe volontaire** à trouver et corriger — en l'état, il ne s'initialise/valide même pas. Le module `ec2` y est laissé en suspens (`// la suite ?`).
- `vpc-module/` : **fourni en exemple** — son `main.tf` est complet (VPC, subnet, IGW, route, SG). Inspire-toi de sa structure. Ses `variables.tf` et `outputs.tf` sont **vides** : à toi de les écrire pour exposer ce dont l'assemblage a besoin.
...
- **AMI** : l'ID écrit en dur dans l'assemblage n'est probablement plus valide / dépend de la région. Ne dépends pas d'une valeur figée — rends-la paramétrable et/ou résous-la dynamiquement.
