...
Un parcours de **13 exercices** pour apprendre Terraform **par la pratique** et **par l'erreur**. Chaque exercice est un défi : on te donne l'**objectif** et les **critères de réussite**, à toi d'écrire la configuration.

> ℹ️ Dépôt pédagogique. Les exercices sont **volontairement incomplets** (`// TODO`, fichiers vides, bugs à corriger). Ce n'est pas un bug du dépôt, c'est le sujet. 😉
...
- Selon l'exercice : un compte **AWS** / **Vultr**, **Python 3**, **Docker**, un serveur **Vault**.
- Les CLIs nécessaires (`aws`, `vault`, `docker`…) installés et configurés.

## 🔒 Règles de sécurité — NON NÉGOCIABLES (valables pour TOUS les exercices)
1. **Jamais** de secret, clé, token, `role_id`/`secret_id` en clair dans un fichier `.tf` ou un fichier commité. Utilise une **variable d'environnement** (`AWS_PROFILE`, `TF_VAR_xxx`, `VULTR_API_KEY`, `VAULT_ADDR`/`VAULT_TOKEN`…) ou un `*.tfvars` **ignoré par git**.
...
3. **Restreins les flux réseau** au strict nécessaire. `0.0.0.0/0` sur SSH (22) ou sur une large plage de ports est un risque : ouvre à ton IP, et seulement le port applicatif au public.
